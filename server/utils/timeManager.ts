...
export const getAppNow = (): string => {
    ...
    // Return strict standardized string WITH Z suffix (UTC)
    // to unify database storage across all tables.
...
export const parseAndFormat = (dateString: string | null, timezone?: string): string => {
    ...
    try {
        ...
        if (dateString.includes('Z') || dateString.match(/[+-]\d{2}:\d{2}$/)) {
            ...
            // Treat SQLite default timestamps (which are UTC but lack 'Z') as UTC implicitly to shift correctly
